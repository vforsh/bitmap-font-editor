...
export class GamePanel extends EditorPanel {
	...
	constructor(scene: Phaser.Scene, container: HTMLElement, config: GamePanelConfig) {
		...
		this.panel.addMonitor(this.config, 'gameDirectory').controller_.view.element.addEventListener('dblclick', () => {
			this.emit(EditorPanelEvent.OPEN_DIRECTORY, this.config.gameDirectory)
		})

		this.panel.addMonitor(this.config, 'fontsDirectory').controller_.view.element.addEventListener('dblclick', () => {
