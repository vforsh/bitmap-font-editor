...
export class PreviewPanel extends EditorPanel {
	...
	constructor(scene: Phaser.Scene, container: HTMLElement, config: PreviewPanelConfig) {
		...
		this.panel.addInput(this.config, 'debug').on('change', (e) => this.emit('debug-change', this.config, e.presetKey))

		this.panel.addInput(this.config, 'debugColor').on('change', (e) => this.emit('debug-change', this.config, e.presetKey))

		this.panel.addSeparator()

		this.panel
			.addInput(this.config, 'align', {
				options: this.getAlignOptions(),
			})
			.on('change', (e) => this.emit('change', this.config, e.presetKey))

		this.panel
			.addInput(this.config, 'maxWidth', {
				step: 10,
				min: 0,
				max: 1000,
			})
		...
		this.panel
			.addInput(this.config, 'letterSpacing', {
				step: 1,
				min: -20,
				max: 20,
			})
		...
		this.panel.addInput(this.config, 'content').on('change', (e) => this.emit('change', this.config, e.presetKey))
