...
export class GlowPanel extends EditorPanel {
	...
	constructor(scene: Phaser.Scene, container: HTMLElement, config: GlowPanelConfig) {
		...
		this.panel.addInput(this.config, 'enabled').on('change', (e) => this.emit('change', this.config, e.presetKey))

		this.panel
			.addInput(this.config, 'distance', {
				step: 1,
				min: 1,
				max: 20,
			})
		...
		this.panel
			.addInput(this.config, 'innerStrength', {
				step: 0.5,
				min: 0,
				max: 30,
			})
		...
		this.panel
			.addInput(this.config, 'outerStrength', {
				step: 0.5,
				min: 0,
				max: 30,
			})
