...
export function createXml(data: BmFontData): XMLDocument {
	...
	const commonDoc = parser.parseFromString(formatStr(TEMP_COMMON, common), 'text/xml')
	xmlDOM.documentElement.appendChild(commonDoc.childNodes[0])

	const pagesDoc = parser.parseFromString(`<pages>${pages.map((p) => formatStr(TEMP_PAGE, p))}</pages>`, 'text/xml')
	xmlDOM.documentElement.appendChild(pagesDoc.childNodes[0])

	const charsDoc = parser.parseFromString(formatStr(TEMP_CHARS, chars), 'text/xml')

	chars.list.forEach((char) => {
		const charDoc = parser.parseFromString(formatStr(TEMP_CHAR, char), 'text/xml')
	...
	if (kernings.count) {
		const kerningsDoc = parser.parseFromString(formatStr(TEMP_KERNINGS, kernings), 'text/xml')

		kernings.list.forEach((kerning) => {
			const kerningDoc = parser.parseFromString(formatStr(TEMP_KERNING, kerning), 'text/xml')
